...
def timestep_embedding(t: Tensor, dim: int = 256) -> Tensor:
  ...
  if dim not in _timestep_freqs_cache:
    _timestep_freqs_cache[dim] = Tensor.exp(-math.log(10000) * Tensor.arange(0, half).half() / half).realize()
...
def train():
  ...
  noises = Tensor.randn(batch, 3, 32, 32).half()
...
def sample_heun(model: Model, n_steps: int = 200, bs: int = 16, eps: float = 1e-3) -> Tensor:
  ...
  x = Tensor.randn(bs, 3, 32, 32).half().realize()

  # Pre-allocate reusable buffers (JIT inputs can't be CONSTs)
  dt_t = Tensor([dt]).half().realize()
  ti_buf = Tensor.zeros(bs).half().contiguous().realize()
  tip1_buf = Tensor.zeros(bs).half().contiguous().realize()

  for i in range(n_steps - 1):
    ti_buf.assign(Tensor.full((bs,), t0 + i * dt).half()).realize()
    tip1_buf.assign(Tensor.full((bs,), t0 + (i + 1) * dt).half()).realize()
    x = _heun_jit(x, ti_buf, tip1_buf, dt_t)

  # final euler step
  ti_buf.assign(Tensor.full((bs,), t0 + (n_steps - 1) * dt).half()).realize()
